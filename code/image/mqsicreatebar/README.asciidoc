...
├── README.asciidoc
├── deps
│   └── 11.0.0.15-ACE-LINUX64-DEVELOPER.tar.gz
└── mqsicreatebar.sh
----
...
[source,bash]
----
$ docker build -t mqsicreatebar:11.0.0.15 \
        --build-arg ACE_INSTALL=11.0.0.15-ACE-LINUX64-DEVELOPER.tar.gz .
----
...
[source,bash]
----
$ docker run mqsicreatebar:11.0.0.15
----
...
== Upload the image to OCP internal registry

In order to use this image in `generate-bar` tekton task, we need to upload it to a container image registry the tekton task has access to. OpenShift's internal registry is such as registry, so we will use that.

. Make sure your instance of OpenShift internal registry is exposed
+
